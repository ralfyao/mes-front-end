<template>
  <q-card class="q-pa-md"  style="width: 1500px; max-width: 100vw;">
    <q-card-section>
      <div class="text-h4">{{mode}}出貨單</div>
    </q-card-section>
    <q-card-section>
      <q-form ref="myForm">
        <!--日期、單號、客戶編號、客戶簡稱、應付佣金-->
        <div class="row q-col-gutter-md">
          ...
          <div class="col-2 col-md-2" style="max-width: 250px">
            <q-select v-model="form.客戶編號" readonly outlined dense label="客戶編號"
        ...
        <div class="row q-col-gutter-md">
          <div class="col-3 col-md-3" style="max-width: 375px">
            <q-input outlined dense label="原定交貨日期" v-model="form.原定交貨日期">
              <template v-slot:append>
                <q-icon name="event" class="cursor-pointer">
        ...
        <div class="row q-col-gutter-md">
          ...
          <div class="col-1 col-md-1" style="max-width: 125px">
            <q-input outlined dense label="訂單總額" v-model="form.總額" readonly/>
          </div>
          <div class="col-6 col-md-6" style="max-width: 750px">
            <q-input outlined dense label="收貨地址" v-model="form.收貨地址"/>
          </div>
        </div>
        <!--收款帳號、指配國別、目的港、交貨方式、應付佣金-->
        <div class="row q-col-gutter-md">
          <div class="col-2 col-md-2" style="max-width: 250px">
            <q-input v-model="form.收款帳號" outlined dense label="收款帳號" readonly/>
            <q-btn outlined dense glossy color="green"  v-if="!preview" label="核對" @click="openBankInfo"/>
          </div>
          <div class="col-2 col-md-2" style="max-width: 250px">
            <q-input v-model="form.指配國別" outlined dense readonly label="指配國別" />
          </div>
          <div class="col-2 col-md-2" style="max-width: 250px">
            <q-input v-model="form.目的港" outlined dense label="目的港"/>
          </div>
          <div class="col-3 col-md-3" style="max-width: 375px">
        ...
        <div class="row q-col-gutter-md">
          ...
          <!--備註說明-->
          <div class="col-4 col-md-4" style="max-width: 500px">
            <q-input outlined dense label="備註說明" v-model="form.remark"/>
          </div>
          <!--付款方式-->
          <div class="col-4 col-md-4" style="max-width: 500px">
            <q-select  outlined dense v-model="form.付款方式" label="付款方式" :readonly="readonly || preview"
                    :options="paymentTerm"
...
const onBlur = (item) =>{
  item.金額2 = item.數量2 * item.單價2;
  form.value.總額 = form.value.shipOrderLists.reduce((sum, current) => sum + current.金額2, 0);
}
...
onMounted(async () => {
  ...
  await custStore.getSalesOrderListByNo(props.salesOrderNo).then((data)=>{
    if (data.length > 0){
      const salesOrder = data[0];
      form.value.客戶編號 = salesOrder.客戶編號;
      form.value.業務員 = salesOrder.業務員;
      form.value.幣別 = salesOrder.幣別;
      form.value.稅別 = salesOrder.稅別;
      form.value.稅率 = salesOrder.稅率;
      form.value.交貨地址 = salesOrder.交貨地址;
      form.value.指配國別 = salesOrder.指配國別;
      form.value.目的港 = salesOrder.目的港;
      form.value.價格條件 = salesOrder.價格條件;
      form.value.交貨方式 = salesOrder.交貨方式;
      form.value.付款方式 = salesOrder.付款方式;
    }
  })
